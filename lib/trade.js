  ...
  try {
    ...
    const tradeResult = {
      ...
      details: {
        ...
        // Input token details
        inputToken: {
          symbol: action === 'buy' ? 'USDC' : 'WSOL',
          amount: action === 'buy' ? tradeAmount / 1e6 : tradeAmount / LAMPORTS_PER_SOL,
          usdValue: action === 'buy' ? tradeAmount / 1e6 : (tradeAmount / LAMPORTS_PER_SOL) * quoteResponse.price
        },
        // Output token details
        outputToken: {
          symbol: action === 'buy' ? 'WSOL' : 'USDC',
          amount: Number(quoteResponse.outAmount) / (action === 'buy' ? LAMPORTS_PER_SOL : 1e6)
        },
      ...
      metadata: {
        ...
        swap: {
          input: {
            ...
            usdValue: action === 'buy' 
              ? tradeAmount / 1e6 
              : ((tradeAmount / LAMPORTS_PER_SOL) * quoteResponse.price).toFixed(2)
          },
          output: {
            amount: Number(quoteResponse.outAmount) / (action === 'buy' ? LAMPORTS_PER_SOL : 1e6),
            symbol: action === 'buy' ? 'WSOL' : 'USDC'
